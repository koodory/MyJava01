...
public class Episode03{
	...
	public static void main02(String[] args) throws Exception {
	  ...
	  System.out.println(len);
		out.write(len >> 24); //끝으로 bit를 밀어넣기 위함, 문자열 길이 출력
		out.write(len >> 16); 
		out.write(len >> 8); 
		out.write(len);
		out.write(bytes); 
	...
	public static void main01 (String[] args) throws Exception {
		...
		byte[] bytes = name.getBytes("UTF-8");
		System.out.println(bytes.length);
		out.write(bytes); //9 byte 출력 => 문자열과 숫자사이의 경계가 없다
		out.write(kor); // 1byte  출력
		out.write(eng); // 1byte 출력
		out.write(math);// 1byte 출력
		out.write(sum); // 1byte 출력 --> 270은 1byte를 초과하므로 문제 발생
		out.close();
	}
}
