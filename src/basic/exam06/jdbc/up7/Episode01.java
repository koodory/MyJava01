...
// 하나의 클래스에 2개의 public 클래스가 존재할 수 없다.
class Patient01{
	String name;
	int age;
	int weight;
...
public class Episode01 {
	...
	public static void main(String[] args){
		...
		/* 유효하지 않은 값이 허용되면 추상화의 의미가 없다.
		 * => 추상화를 한 의미가 없다
		 * => 시스템 실행시 치명적인 업무 오류를 유발한다.
		 * 
		 * 해결책: 캡슐화
		 * => 유효하지 않은 값을 가지는 것을 방지
		 * => 잘못된 메서드 호출을 방지
		 * => 문법: private, (default), protected, public
		 * private 클래스 멤버(변수와 메서드)만 가능
		 * (default): 클래스 멤머 + 패키지 멤버도 가능
		 * protected: 클래스 멤머 + 패키지 멤머 + 자식 클래스
		 * public 모두
		 */
	}
}
